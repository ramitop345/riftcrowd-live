...
function startProcess(
  ...
  const managed: ManagedProcess = { name, process: child };
  ...
  child.on('exit', (code, signal) => {
    ...
    if (idx >= 0) managedProcesses.splice(idx, 1);
  });
...
async function waitForHealth(url: string, timeoutMs: number = 30000): Promise<boolean> {
...
async function main(): Promise<void> {
  ...
  const gatewayProcess = startProcess(
    'gateway',
    'node',
    [gatewayEntry],
    gatewayEnv,
  ...
  if (!args.skipGodot) {
    if (existsSync(godotExe)) {
      ...
      const godotProcess = startProcess(
        'godot',
        godotExe,
        godotArgs,
        {},
        join(logDir, 'godot.log'),
      );
      void godotProcess;
    } else {
...
export type { LauncherArgs, ManagedProcess };
