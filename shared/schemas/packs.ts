...
export const CONTENT_PACK_MODES: readonly ContentPackMode[] = ContentPackModeSchema.options;
 ...
 */
export function buildKeywordIndex(pack: ContentPack): Map<string, string> {
  ...
  for (const faction of pack.factions) {
    for (const keyword of faction.joinKeywords) {
      ...
      if (owner !== undefined && owner !== faction.id) {
        throw new Error(
          `Join keyword collision in pack "${pack.id}": "${lowered}" claimed by both "${owner}" and "${faction.id}".`,
        );
      }
 ...
 */
const keywordIndexCache = new WeakMap<ContentPack, Map<string, string>>();

function getKeywordIndex(pack: ContentPack): Map<string, string> {
 ...
 */
export function matchJoinKeyword(pack: ContentPack, rawText: string): string | null {
