...
import { createAndRegisterDirector } from './director/index.js';
...
export function buildApp(options = {}) {
    ...
    if (options.enableDirector) {
        ...
        try {
            ...
            const viewerParsed = JSON.parse(viewerRaw);
    ...
    if (enablePipeline && pipeline) {
        ...
        const runtimeConfigMirror = sanitizeConfig(config);
        ...
        if (options.enableGiftEconomy === true) {
            try {
                ...
                const getFaction = (viewerId) => {
                    const profile = director?.viewerRegistry?.get(viewerId);
                    return profile?.factionId ?? null;
                };
                const giftEconomy = new GiftEconomy(giftConfig, undefined, (msg) => app.log.info(msg), getFaction);
        ...
        if (options.enableFreeEngagement === true) {
            try {
                ...
                const freeEngagement = new FreeEngagement(freeEngagementConfig, (msg) => app.log.info(msg), getFaction);
        ...
        if (options.enableRunbook === true) {
            try {
                ...
                // Provider check
                preflight.addCheck(makeProviderCheck(config.liveProvider, () => true, // Mock adapter is always running if provider=mock
                async () => {
        ...
        if (options.enableWs === true) {
            ...
            let fallbackDrainInterval = null;
