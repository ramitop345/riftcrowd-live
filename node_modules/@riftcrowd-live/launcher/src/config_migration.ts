...
function resolveConfigDir(): string {
  ...
  if (existsSync(releaseConfigDir)) return releaseConfigDir;
  ...
  if (existsSync(devConfigDir)) return devConfigDir;
 ...
 */
function createBackup(configDir: string, fileName: string, timestamp: string): string | null {
...
function migrateConfigFile(
  ...
  def: ConfigFileDef,
  timestamp: string,
): { migrated: boolean; skipped: boolean; error: string | null; backupPath: string | null } {
 ...
 */
export function runConfigMigration(configDir?: string): MigrationResult {
 ...
 */
export function listBackups(configDir?: string): string[] {
  const resolvedDir = configDir ?? resolveConfigDir();
