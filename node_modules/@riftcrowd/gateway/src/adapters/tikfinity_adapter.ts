...
export type TikFinityConfig = z.infer<typeof TikFinityConfigSchema>;
...
export type TikFinityRawEvent = z.infer<typeof TikFinityRawEventSchema>;
 ...
 */
export function parseTikfinityPayload(raw: string): { ok: true; value: TikFinityRawEvent } | { ok: false; error: string } {
  try {
...
// ---------------------------------------------------------------------------
// TikFinityAdapter class
// ---------------------------------------------------------------------------

export interface TikFinityAdapterOptions {
  ...
  onInfo?: (msg: string, fields?: Record<string, unknown>) => void;
  /** Optional callback when adapter permanently gives up after max reconnect. */
  onFatal?: (message: string) => void;
}

export class TikFinityAdapter implements LiveAdapter {
  private readonly cfg: TikFinityConfig;
  private readonly onWarn: (msg: string, fields?: Record<string, unknown>) => void;
  private readonly onInfo: (msg: string, fields?: Record<string, unknown>) => void;
  ...
  private readonly instanceSalt = randomUUID().slice(0, 8);
  ...
  private _eventsDropped: number = 0;
  private _reconnectCount: number = 0;

  constructor(opts: TikFinityAdapterOptions) {
   ...
   */
  mapEvent(raw: TikFinityRawEvent): NormalizedLiveEvent | null {
    try {
      this.eventCounter++;
  ...
  private scheduleReconnect(): void {
    ...
    const backoffMs = Math.min(
      this.cfg.reconnectMs * Math.pow(2, this.reconnectAttempts),
      MAX_BACKOFF_MS,
    );
