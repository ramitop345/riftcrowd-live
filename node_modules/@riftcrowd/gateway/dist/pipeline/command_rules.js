...
// ---------------------------------------------------------------------------
function makeCommand(type, event, extra) {
    return {
...
export class CommandRulesEngine {
     ...
     */
    static extractKickTarget(event) {
        if (event.type !== 'chat' || !event.comment)
            return null;
        const trimmed = event.comment.trim();
        if (!trimmed.toLowerCase().startsWith('!kick '))
            return null;
