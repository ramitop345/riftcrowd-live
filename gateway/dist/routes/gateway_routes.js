...
export function registerGatewayRoutes(app, opts) {
    ...
    app.post('/config', (request, reply) => {
        ...
        if (runtimeConfigMirror) {
            const pipelineSection = (runtimeConfigMirror['pipeline'] ?? {});
            if (update.rateLimitPerViewer !== undefined)
                pipelineSection['rateLimitPerViewer'] = update.rateLimitPerViewer;
            if (update.rateLimitBurst !== undefined)
                pipelineSection['rateLimitBurst'] = update.rateLimitBurst;
            if (update.rateLimitGlobal !== undefined)
                pipelineSection['rateLimitGlobal'] = update.rateLimitGlobal;
            if (update.dedupeCapacity !== undefined)
                pipelineSection['dedupeCapacity'] = update.dedupeCapacity;
            if (update.commandQueueCapacity !== undefined)
                pipelineSection['commandQueueCapacity'] = update.commandQueueCapacity;
            if (update.eventBusCapacity !== undefined)
                pipelineSection['eventBusCapacity'] = update.eventBusCapacity;
            runtimeConfigMirror['pipeline'] = pipelineSection;
            if (update.logLevel !== undefined)
                runtimeConfigMirror['logLevel'] = update.logLevel;
        }
