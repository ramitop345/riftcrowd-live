...
function collectRedactedConfig(cfg) {
    return sanitizeConfig(cfg);
}
/** Read recent log lines from log directory. */
function collectRecentLogs(logDir, maxLines = 1000) {
    const lines = [];
 ...
 */
async function createDiagnosticZip(logDir, cfg) {
...
export function registerDiagnosticsRoute(app, opts) {
