...
function makeHash(id, extra = '') {
    return `sha256:mock_${id}_${extra}`.slice(0, 128);
}
export function makeChatEvent(opts) {
    const id = nextId();
    return {
        timeMs: opts.timeMs,
        event: {
...
export function makeGiftEvent(opts) {
    const id = nextId();
    return {
        timeMs: opts.timeMs,
        event: {
...
export function makeLikeEvent(timeMs, viewerId, count = 1) {
    const id = nextId();
    return {
        timeMs,
        event: {
...
export function makeJoinEvent(timeMs, viewerId) {
    const id = nextId();
    return {
        timeMs,
        event: {
 ...
 */
export function makeMalformedEvent(timeMs, kind) {
    const id = nextId();
...
// ---------------------------------------------------------------------------
export function malformedPayloads() {
    resetEventCounter();
    const events = [];
    const durationMs = 30_000;
