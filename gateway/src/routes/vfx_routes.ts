...
import type { VFXOrchestrator } from '../vfx/vfx_orchestrator.js';
...
export interface RegisterVFXRoutesOptions {
  orchestrator: VFXOrchestrator;
}

export function registerVFXRoutes(
  app: FastifyInstance,
  opts: RegisterVFXRoutesOptions,
): void {
  ...
  app.post('/vfx/trigger', (request, reply) => {
    ...
    const event = {
      ...
      provider: 'mock',
      type: eventType as 'like' | 'follow' | 'share' | 'gift' | 'subscribe' | 'chat',
