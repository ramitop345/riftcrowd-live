...
export const WINDOW_DEFAULTS: WindowConfig = {
  mode: 'windowed',
  portrait: true,
  width: 1080,
  height: 1920,
...
export function loadWindowConfig(configPath?: string): WindowConfig {
  const resolvedPath =
    configPath ??
 ...
 */
export function reloadWindowConfig(configPath?: string): {
  ok: true;
  config: WindowConfig;
} | {
  ok: false;
