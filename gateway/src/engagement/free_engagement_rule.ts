...
import type { SpamFilter } from './spam_filter.js';
import type { LikeMilestoneAggregator } from './like_milestone_aggregator.js';
import type { FollowGuardian } from './follow_guardian.js';
import type { ShareShield } from './share_shield.js';
import type { StrategyVote } from './strategy_vote.js';
import type { FreeEnergyAbility } from './free_energy_ability.js';
import type { TopContributor } from './top_contributor.js';
...
export class FreeEngagementRule implements Rule {
  readonly name = 'FreeEngagementRule';

  private readonly spamFilter: SpamFilter;
  private readonly likeAggregator: LikeMilestoneAggregator;
  private readonly followGuardian: FollowGuardian;
  private readonly shareShield: ShareShield;
  private readonly strategyVote: StrategyVote;
  private readonly freeEnergyAbility: FreeEnergyAbility;
  private readonly topContributor: TopContributor;
  private readonly logFn: (msg: string) => void;

  constructor(
    spamFilter: SpamFilter,
    likeAggregator: LikeMilestoneAggregator,
    followGuardian: FollowGuardian,
    shareShield: ShareShield,
    strategyVote: StrategyVote,
    freeEnergyAbility: FreeEnergyAbility,
  ...
  applies(event: NormalizedLiveEvent): boolean {
    if (event.type === 'like' || event.type === 'follow' || event.type === 'share') return true;
