 ...
 */
function sanitizeProviderHandle(raw: string): string {
...
export class ViewerRegistry {
  private readonly profiles = new Map<string, ViewerProfile>();
  private readonly displayNameMaxLength: number;

  constructor(displayNameMaxLength: number = 64) {
  ...
  getOrCreate(
    ...
    const profile = createViewerProfile(
      viewerId,
      sanitizeProviderHandle(providerHandle),
      rawDisplayName,
  ...
  get(viewerId: string): ViewerProfile | undefined {
